...
$counts = [
    ...
    'total_sellers' => "SELECT COUNT(*) AS total FROM users WHERE role='Farmer' AND status='approved'", // Updated: Count only approved sellers
    'total_buyers' => "SELECT COUNT(*) AS total FROM users WHERE role='Buyer'",
    'pending_sellers' => "SELECT COUNT(*) AS total FROM users WHERE role='Farmer' AND status='Pending'",
];;
