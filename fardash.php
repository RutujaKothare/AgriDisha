...
$stats_query = "SELECT 
    ...
    (SELECT COALESCE(SUM(total_price), 0) FROM orders WHERE seller_id = ? AND status = 'Delivered') as total_revenue,
    (SELECT COALESCE(SUM(t.amount), 0) 
     FROM transactions t 
     JOIN orders o ON t.order_id = o.id 
     WHERE o.seller_id = ? AND t.payment_status = 'unpaid') as pending_payments";
